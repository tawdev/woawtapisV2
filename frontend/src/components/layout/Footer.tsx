            ...
            <div className="container mx-auto px-4">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-16 mb-20 text-center md:text-left">
                    ...
                    <div className="space-y-8">
                        ...
                        <div className="flex justify-center md:justify-start gap-6">
                            {['Instagram', 'Pinterest', 'Facebook'].map(social => (
                                <a key={social} href="#" className="text-stone-500 hover:text-primary transition-colors text-xs uppercase tracking-widest font-bold">{social}</a>
                            ))}
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <ul className="space-y-4 text-stone-400 text-sm">
                            {['Nos Tapis', 'Sur Mesure', 'Collections', 'À Propos', 'Journal'].map(link => (
                                <li key={link}><a href={link === 'Journal' ? '/blog' : '#'} className="hover:text-white transition-colors">{link}</a></li>
                            ))}
                        </ul>
                    </div>
                    ...
                    <div className="space-y-8">
                        ...
                        <p className="text-stone-400 text-sm">Inscrivez-vous pour recevoir nos sélections privées.</p>
                        <form className="relative">
                            ...
                            />
                            <button className="absolute right-0 top-1/2 -translate-y-1/2 text-primary font-bold uppercase text-[10px] tracking-widest hover:text-white transition-colors">OK</button>
                        </form>
                    </div>
                </div>

                <div className="pt-12 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-8 text-[10px] uppercase tracking-[0.2em] font-bold text-stone-500">
